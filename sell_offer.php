...
function gen_code ()
{
  ...
  $query  = "select `id` from `sell_offers` ORDER BY `id` DESC LIMIT 1";
...
if(isset($_POST['product']))
{
    ...
    if(count($errors) == 0)
    {
        ...
        $query = "INSERT INTO `sell_offers` ( `code`,
                `product_name`, `quantity`, `price`, `packaging`,
                `contact_name`, `contact_email`, `tel`, `company`, `user_id`,
                `day`, `month`, `year`, `time_added`, `date`, `description`
            )

            VALUES ( '$code',
                '$product', '$quantity', '$price',  '$packaging',
        ...
        $success = "Sell Offer Made. You will be contacted when it is approved";
    }
...
<div class="row">
    <div class="col-md-12">
        <h2 class="page-header" >Sell Offer</h2>
