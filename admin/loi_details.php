...
if(isset($_GET['status']))
{
    ...
    $query = "UPDATE `loi` SET `status` = '$statu' WHERE `id` = '$id' ";
    $result = mysqli_query($dbc, $query)
        or die("Error, Could not complete the action");

    //alert the user
    $success = "LOI Status Changed Successfully";
}
...
if(isset($_GET['file']))
{
    ...
    if($action == 'true')
    {
        ...
        $query = "SELECT * FROM `loi_files` WHERE `id` = '$file' ";
        ...
        if(file_exists($file_location))
        {
            if(unlink($file_location))
            {
                ...
                $query = "DELETE FROM `loi_files` WHERE `id` = '$file' ";
                $result = $dbc->query($query);
    ...
    if(!empty($file_name))
    {
        ...
        if(count($errors) == 0)
        {
            ...
            if(move_uploaded_file($tmp_name, $file_location))
            {
                ...
                $query  = "INSERT INTO `loi_files` (`loi_id`, `file_name`, `size`, `location`,
                                    `type`, `user_id`)

                                VALUES ('$loi_id', '$file_name', '$file_size', '$file_location',
                                    '$type', '$user_id'
                                )
                                    ";
                $result = $dbc->query($query);
...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="card-box">
            ...
            <div class="row">
                ...
                <?php
                $query = "SELECT * FROM `loi_files` WHERE `loi_id` = '$id'";
                $result = $dbc->query($query);
    ...
    <div class="col-md-12">
        <div class="text-center">
            ...
            $query = "SELECT `id` FROM `loi` WHERE `id` < $id ORDER BY `id` DESC LIMIT 1";
            $result = mysqli_query($dbc, $query)
                or die("Error.");
            ...
            {
                ?>
            <a href="loi_details.php?id=<?php echo $id; ?>&status=<?php echo Status::AVAILABLE; ?>" class="btn btn-success">
            ...
            {
                ?>
            <a href="loi_details.php?id=<?php echo $id; ?>&status=<?php echo Status::CLOSED; ?>" class="btn btn-danger">
            ...
            {
                ?>
            <a href="loi_details.php?id=<?php echo $id; ?>&status=<?php echo Status::PENDING; ?>" class="btn btn-warning">
